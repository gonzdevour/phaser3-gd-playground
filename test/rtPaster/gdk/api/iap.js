...
class cdv_purchase extends EE {
  ...
  init() {
    ...
    if (store) {
      ...
      store.validator = this.validatorLink?this.validatorLink:undefined;
      // Add When events
...
function iapInit(validatorLink) {
  var iap;
  if (OS.cordova && (OS.iOS || OS.android)) {
    log("init cdv purchase plugin");
    iap = new cdv_purchase(validatorLink);
    iap.init();
  } else {
    log("use web purchase");
    iap = new p3_purchase(validatorLink);
  }
  return iap;
}
